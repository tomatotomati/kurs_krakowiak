...
img = cv2.imread(f'assets\poland.png')
# cv2.imshow('img', img)
...
img = cv2.drawContours(image=img, contours=[contours[0]], contourIdx=-1, color=(0, 255, 0), thickness=1 )
...
for point in [leftmost, rightmost, topmost, bottommost]:
    cv2.circle(img=img, center=tuple(point), radius=5, color=(0, 0, 255), thickness=-1)
